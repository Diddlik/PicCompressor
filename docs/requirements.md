...
Das Feld `historyWarning` meldet einen Persistenzfehler des Verlaufs. Es ist `null`, wenn der Verlauf geschrieben wurde oder über `--no-history` abgeschaltet ist, und verändert den Exit-Code nicht.
...
- Einstellungen liegen als eine versionierte JSON-Datei im plattformgerechten Benutzer-Konfigurationsverzeichnis, getrennt von Verlauf und Logs im Anwendungsdatenverzeichnis.
- Unbekannte oder beschädigte Konfiguration führt zu einer verständlichen Diagnose und sicheren Defaults.
- Die Ersetzung erfolgt feldweise: ein einzelner unzulässiger Wert darf die übrigen Einstellungen nicht verwerfen. Nur eine unlesbare Datei oder eine neuere Schemaversion führt zu vollständigen Vorgabewerten.
- Aufzählungen werden unter ihrem Namen gespeichert, nicht als Zahl; eine Umsortierung darf eine gespeicherte Konfiguration nicht umdeuten.
- Geschrieben wird erst vollständig in eine temporäre Datei und dann ersetzt, damit ein Abbruch keine halbe Konfiguration hinterlässt.
- Ein Schreibfehler wird gemeldet, darf die Anwendung aber nicht beenden.
...
| Solution und Projekte | Teilweise verifiziert | Windows x64: .NET SDK 10.0.302; normaler Solution-Build erzeugt den MSVC-`RelWithDebInfo`-Wrapper inkrementell, führt dessen ABI- und EXIF-Tests aus und kopiert DLL/PDB transitiv zu Desktop, CLI und Testhost; vollständige Solution mit 193 Tests am 2026-07-20 verifiziert |
...
| O-008 | Bedienoberflaeche fuer Aufbewahrungsdauer und Grenzwerte der Logrotation | die Aufbewahrung ist ueber `HistoryRetentionDays` in der Konfigurationsdatei einstellbar und wird beim Start angewandt; die Logrotation nutzt weiterhin feste Werte (5 MB, 5 Generationen). Offen ist, welche dieser Werte die Oberflaeche zeigen soll |
...
| D-024 | 2026-07-20 | Einstellungen liegen in einer einzigen versionierten JSON-Datei; unzulaessige Werte werden feldweise durch Vorgabewerte ersetzt, nicht die ganze Datei verworfen | Abschnitt 13.2 fordert sichere Defaults bei beschaedigter Konfiguration. Ein einzelner unsinniger Wert darf nicht alle uebrigen Einstellungen des Benutzers loeschen |
| D-025 | 2026-07-20 | Die GUI kennt den Application-Port fuer Einstellungen, nicht die Infrastruktur; der Desktop Host verdrahtet den JSON-Speicher | die Schichtregel aus 14.1 erlaubt der GUI Domain und Application, aber keine Infrastruktur |
